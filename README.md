...
一个基于 Python 3.12 的 OpenRouter 飞书机器人，按用户 `open_id` 隔离管理多个 Key，支持长连接收消息、个人化阈值配置、每日/间隔主动私聊推送和阈值告警。
...
- 单用户可绑定多个 Key，并可设置别名
- 每个用户有独立的每日推送时间、间隔推送设置、免打扰时段和 `warning/danger/critical` 阈值
- 每日主动私聊推送 `/详细` 完整报告
- 定时阈值扫描，命中后主动私聊告警
...
- `data/runtime_state.json`
  - 存储告警去重状态、失败计数、每日推送日期、下次间隔推送时间
- `openrouter_monitor/service.py`
  - 用户绑定、删除、配置更新、阈值扫描、每日/间隔主动推送
...
service:
  ...
  # 可选。用户间隔推送静默窗默认值；用户后续可通过 /配置 间隔静默 覆盖或关闭。
...
defaults:
  push_time: "09:00"
  # 可选。给新绑定用户设置默认间隔推送，单位为分钟；不填则默认关闭。
...
- `defaults.push_interval_minutes` 只用于“新绑定用户的初始值”，不是全局强制推送频率
- `service.interval_quiet_hours` 是“用户免打扰时段默认值”，只影响间隔推送，不影响每日固定时间推送
- 用户可通过 `/配置 间隔静默 HH:MM HH:MM` 覆盖默认值，也可通过 `/配置 间隔静默 关闭` 单独关闭
...
- 用户可通过 `/配置 间隔 <minutes>` 开启间隔主动推送
- 第一次发送从“保存间隔配置成功”的时刻开始计时；如果用户先配置了间隔、后绑定第一把 Key，则从绑定成功时开始计时
- 每次成功发送后，下一次发送时间会顺延 `N` 分钟
- 如果发送失败，不会推进下一次发送时间，下一轮分钟检查会继续重试
- 默认情况下会继承 `service.interval_quiet_hours`
- 用户也可以通过 `/配置 间隔静默 HH:MM HH:MM` 单独设置自己的静默时间段，或通过 `/配置 间隔静默 关闭` 关闭
- 命中静默时间段的间隔推送会自动跳过，并在静默时间结束后恢复发送

### 同一分钟命中两种规则

- 如果每日推送和间隔推送恰好落在同一分钟，机器人会连续发送两条 `/详细`
- 两条消息内容相同，但会分别记账各自的运行状态
...
这个命令用于验证主动消息链路是否可用，发送时会在消息末尾自动追加“开发调试，如果不是开发请忽略”，方便和真实主动消息区分。
